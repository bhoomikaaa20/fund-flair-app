...
function Profile() {
  ...
  // Sign a URL for private avatar bucket
  ...
  const saveProfile = useMutation({
    mutationFn: async () => {
      if (!uid) return;
      const { error } = await supabase.from("profiles").update({ full_name: fullName, phone }).eq("id", uid);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Profile updated"); qc.invalidateQueries({ queryKey: ["profile", uid] }); },
  ...
  const uploadAvatar = async (file: File) => {
    ...
    const { error: upErr } = await supabase.from("profiles").update({ avatar_url: path }).eq("id", uid);
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-6">
          <div className="relative">
            <div className="grid size-20 place-items-center overflow-hidden rounded-full bg-secondary text-2xl font-medium text-muted-foreground">
              {signedAvatar ? <img src={signedAvatar} alt="" className="size-full object-cover" /> : (fullName?.[0]?.toUpperCase() ?? "?")}
            </div>
            <button
              type="button"
      ...
      <form className="space-y-4 rounded-2xl border bg-card p-6" onSubmit={(e) => { e.preventDefault(); saveProfile.mutate(); }}>
        ...
        <Button type="submit" disabled={saveProfile.isPending}>{saveProfile.isPending ? "Saving…" : "Save changes"}</Button>
      </form>

      <form className="space-y-4 rounded-2xl border bg-card p-6" onSubmit={(e) => { e.preventDefault(); changePassword.mutate(); }}>
        <h2 className="font-medium">Change password</h2>
        <div className="max-w-sm space-y-1.5">
          <Label>New password</Label>
          <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={6} maxLength={72} />
        </div>
        <Button type="submit" variant="outline" disabled={changePassword.isPending || !password}>
